...
namespace TempestSushi.Web.Controllers
{
    public class ProductoController : Controller
    {
        ...
        {
            if (dto.Imagen == null)
                ModelState.AddModelError(nameof(dto.Imagen), "Debe adjuntar una imagen del producto.");

            if (await _service.ExisteNombreAsync(dto.Nombre))
                ModelState.AddModelError(nameof(dto.Nombre), "Ya existe un producto registrado con ese nombre.");
            ...
            TempData["Mensaje"] = $"Producto \"{dto.Nombre}\" creado correctamente.";
        ...
        {
            if (id != dto.IdProducto) return NotFound();

            if (await _service.ExisteNombreAsync(dto.Nombre, dto.IdProducto))
                ModelState.AddModelError(nameof(dto.Nombre), "Ya existe otro producto registrado con ese nombre.");
            ...
            string? imagenUrl = dto.Imagen != null ? await GuardarImagenAsync(dto.Imagen) : null;
            var actualizado = await _service.ActualizarAsync(dto, imagenUrl);
            if (!actualizado) return NotFound();

            TempData["Mensaje"] = $"Producto \"{dto.Nombre}\" actualizado correctamente.";
            return RedirectToAction(nameof(Detalle), new { id = dto.IdProducto });
        }
